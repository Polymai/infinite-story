...
function firstChapterBody({ title, genre, premise, language }) {
  if (language === "sv") {
    return [
      `${title} börjar med ett val som redan väntar i luften.`,
      `Världen är ${genre}, men den beter sig inte som en genre. Den andas, svarar och håller räkning. ${premise}`,
      "Först förändras något litet: ett ljud bakom en stängd dörr, ett meddelande i marginalen, ett ljus där inget ljus borde finnas. Du förstår direkt att nästa steg kommer att få berättelsen att minnas dig.",
    ].join("\n\n");
  }
...
function starterChoices(language = "en") {
  if (language === "sv") {
    return [
      { label: "Gå in tyst", text: "Rör dig framåt utan att störa scenen." },
      { label: "Ställ en fråga", text: "Ropa ut och se vem som svarar." },
      { label: "Ta ledtråden", text: "Plocka upp det märkliga föremålet innan någon märker det." },
    ];
  }
...
export async function createStory(input) {
  ...
  const { data, error } = await appDb.rpc("create_story_with_first_chapter", {
    ...
    p_premise: premise,
    p_tone: tone,
    p_body: firstChapterBody({ title, genre, premise, language }),
    p_choices: starterChoices(language),
  });
  if (error) throw error;
  return data;
}
